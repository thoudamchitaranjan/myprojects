...
<#
.Synopsis
    Rejects only the lines containing the value of the $Substring from the list
fetched from the file $Path. The result is stored in -Destination file path.
#>
param (
        [Parameter(mandatory=$true,
        Position=0,
        HelpMessage="The list file which contains the list for Rejecting wanted lines.")]
...
Process{
    ...
    if((-not [string]::IsNullOrWhiteSpace($Path)) -and ($false -ieq (Test-Path "$Path"))){
        Write-Error -Message "List file for Rejecting lines does not exist." -Verbose -InformationAction Continue
        Return $null
    }
    elseif(($DeletePrevious) -and ([string]::IsNullOrWhiteSpace($Destination))){
...
if($ReplaceOriginal){
        ...
        Write-Information -MessageData "Lines with '$Substring' in the file have been Rejected." -Verbose -InformationAction Continue
    }
    else
    {
        Write-Information -MessageData "Lines Rejection canceled." -Verbose -InformationAction Continue
    }
}###Process