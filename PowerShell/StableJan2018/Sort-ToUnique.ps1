...
param (
    ...
    [ValidateScript({-not $ReplaceOriginal})]
    [string]$Destination,
    ...
    [ValidateScript({[string]::IsNullOrWhiteSpace($Destination) })]
    ...
    Process{
        ...
        elseif(($DeletePrevious) -and ([string]::IsNullOrWhiteSpace($Destination))){
            Write-Warniing -Message "Exiting to avoid replacing original file."  -Verbose -InformationAction Continue
            Return $null
        }
        ...
        elseif($DeleteOriginal){
            if((Test-Path $Path) -and ([string]::IsNullOrWhiteSpace($Destination))){
                ...
                Move-Item $tempFilePath -Destination $Path | Out-Null
            }
            elseif(-not [string]::IsNullOrWhiteSpace($Destination)){
                Move-Item $tempFilePath -Destination $Destination | Out-Null
            }
        }
        else{
